...
<script>
    function show_banner(b_name, b_id) {
        ...
        /*
        $.get("<?php echo  Yii::app()->createUrl('admin/banners/show') ?>?b_name=" + b_name + "&b_id=" + b_id, 
        function (data){
            $('#banner_viewer_' + b_name).html(data);
        });
        /**/
        $('#banner_viewer_' + b_name).html("<iframe width='100%' src='<?php echo  Yii::app()->createUrl('/admin/banners/show') ?>?b_name=" + b_name + "&b_id=" + b_id + "'></iframe>");
    }

    function create_block() {
        var bl_name = prompt('Укажите id блока');
        if(bl_name!=null) {
            window.location = '<?php echo  Yii::app()->createUrl('admin/banners') ?>?create_block=yes&new_name=' + bl_name;
        }
    }
    ...
    function delete_ads(bl_name, ads_id) {
        ...
        console.log(cf);
        if (cf) {
            window.location = '<?php echo  Yii::app()->createUrl('admin/banners') ?>?delete_name=' + bl_name + '&delete_id=' + ads_id;
        }
    }
    ...
    function delete_block(bl_name) {
        ...
        if (cf) {
            window.location = '<?php echo  Yii::app()->createUrl('admin/banners') ?>?delete_block=' + bl_name;
        }
    }
...
<div align='center' class='add_code_form'>
    <a href='<?php echo Yii::app()->createUrl("/admin/default/help")?>#banners'> Инструкция </a>
<form method='post' name='insert_ads_form' action='<?php echo  Yii::app()->createUrl('admin/banners') ?>' 
